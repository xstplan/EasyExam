...
using XST.Service.Infrastructure.Extensions;
namespace XST.Service.Repository
{
    public class BaseRepository<T> : SimpleClient<T> where T : class, new()
    {
        public BaseRepository(ISqlSugarClient context = null) : base(context)
        {
            Context = DbScoped.SugarScope.GetConnectionScope(0);
        }
        #region add
        ...
        public PagedInfo<T> GetPaged(PagerInfo parm, Expression<Func<T, object>> order, OrderByType orderEnum = OrderByType.Asc)
        {
            var source = Context
                   .Queryable<T>()
                   .OrderByIF(orderEnum == OrderByType.Asc, order, OrderByType.Asc)
